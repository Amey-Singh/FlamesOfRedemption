...
public class PlayerController : MonoBehaviour, PlayerInputAction.IGameplayActions
{
    ...
    public float horizontalMovementThreshold = 0.5f;
    ...
    public float ledgeClimbCooldown = 2.0f;
    ...
    private float ledgeClimbCooldownTimer = 0f;
    ...
    private bool jumpInputPressed; // Add this new variable
    ...
    private void Update()
    {
        ...
        if (ledgeClimbCooldownTimer > 0)
            ledgeClimbCooldownTimer -= Time.deltaTime;
    ...
    public void OnMovement(InputAction.CallbackContext context)
    {
        ...
        // Apply threshold for horizontal movement
        if (Mathf.Abs(moveInput.x) < horizontalMovementThreshold)
        {
            moveInput.x = 0;
        }
    ...
    {
        if (ledgeDetected && !canClimbLedge && ledgeClimbCooldownTimer <= 0)
            InitiateLedgeClimb();
    ...
    public void FinishLedgeClimb()
    {
        ...
        ledgeDetected = false;
        anim.SetBool("canClimbLedge", canClimbLedge);
        ledgeClimbCooldownTimer = ledgeClimbCooldown;
    ...
    private void CheckJump()
    {
        ...
        if (checkJumpMultiplier && !jumpPressed && !jumpInputPressed)
        {
    ...
    private void WallJump()
    {
        if (canWallJump)
        {
            // Calculate wall jump direction based on facing direction
            // Invert the horizontal component of the wallJumpDirection based on the wall's position
            int wallJumpDirectionX = isFacingRight ? -1 : 1;
            Vector2 forceToAdd = new Vector2(wallJumpDirectionX * wallJumpForce * wallJumpDirection.x, wallJumpForce * wallJumpDirection.y);

            // Apply the calculated force to the player's Rigidbody
